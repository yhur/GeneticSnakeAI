...
class NeuralNetworkViz(QtWidgets.QWidget):
    ...
    def show_network(self, painter: QtGui.QPainter):
        ...
        for v in range(8):
            tick = 'V' + str(v)
            painter.drawText(10, v * 72 + 38, tick)
        for i, l in enumerate(['HU', 'HD', 'HL', 'HR', 'TU', 'TD', 'TL', 'TR']):
            painter.drawText(10, i * 24 + 614, l)

        # Draw nodes
        for layer, num_nodes in enumerate(layer_nodes):
            v_offset = int((height - ((2*radius + vertical_space) * num_nodes))/2)
            activations = None
            if layer > 0:
            ...
            for node in range(num_nodes):
                ...
                # Output layer
                    ...
                    text = ('U', 'D', 'L', 'R')[node]
                    painter.drawText(h_offset + 30, node * (radius*2 + vertical_space) + v_offset + int(1.5*radius), text)
                    if node == max_out:
                        painter.setBrush(QtGui.QBrush(Qt.green))
                    else:
                        painter.setBrush(QtGui.QBrush(Qt.white))
