...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, settings, show=True, fps=200):
        ...
        stored_indv = 0
        if os.path.exists(f'{data_folder}/indv0'):
            for indv in os.listdir(data_folder):
                if os.path.isdir(f'{data_folder}/{indv}'):
                    print(f"loading the generation : {indv}")
                    individual = load_snake(f'./{data_folder}', indv, None)
                    individuals.append(individual)
                    stored_indv += 1

        num_parents = self.settings['num_parents'] - stored_indv
        ...
        if os.path.exists(f'{data_folder}/generation'):
            with open(f'{data_folder}/generation', 'r') as gen:
                v1, v2, v3 = gen.read().split(':')
                self.current_generation = int(v3)
                b1, b2 = v2.split('@')
                self.best_score =  int(b1)
                self.next_generation()
                self.ga_window.best_score_label.setText(str(self.best_score))
    ...
    def next_generation(self):
        ...
        i = 0
        for individual in self.population.individuals:
            save_snake(f'./{data_folder}', 'indv' + str(i), individual, self.settings)
            i += 1
 
        with open(f'{data_folder}/generation', 'w') as gen:
            current_gen = str(self.current_generation - 1)
            gen.write(f'best score : {self.best_score} @ gen : {current_gen}')
...
class SnakeWidget(QtWidgets.QWidget):
    ...
    def draw_snake(self, painter: QtGui.QPainter) -> None:
        ...
        def _draw_line_to_apple(painter: QtGui.QPainter, start_x: int, start_y: int, drawable_vision: DrawableVision) -> Tuple[int, int]:
            ...
            end_x = int(drawable_vision.apple_location.x * SQUARE_SIZE[0] + SQUARE_SIZE[0]/2)
            end_y = int(drawable_vision.apple_location.y * SQUARE_SIZE[1] + SQUARE_SIZE[1]/2)
            painter.drawLine(int(start_x), int(start_y), end_x, end_y)
            return end_x, end_y
        ...
        def _draw_line_to_self(painter: QtGui.QPainter, start_x: int, start_y: int, drawable_vision: DrawableVision) -> Tuple[int, int]:
            ...
            end_x = int(drawable_vision.self_location.x * SQUARE_SIZE[0] + SQUARE_SIZE[0]/2)
            end_y = int(drawable_vision.self_location.y * SQUARE_SIZE[1] + SQUARE_SIZE[1]/2)
            painter.drawLine(int(start_x), int(start_y), end_x, end_y)
            return end_x, end_y
            ...
            if self.snake._drawable_vision[0]:
                for drawable_vision in self.snake._drawable_vision:
                    ...
                    if drawable_vision.wall_location:
                        painter.setPen(QtGui.QPen(Qt.black))
                        end_x = int(drawable_vision.wall_location.x * SQUARE_SIZE[0] + SQUARE_SIZE[0]/2)
                        end_y = int(drawable_vision.wall_location.y * SQUARE_SIZE[1] + SQUARE_SIZE[1]/2)
                        painter.drawLine(int(start_x), int(start_y), end_x, end_y)
    ...
    def keyPressEvent(self, event: QtGui.QKeyEvent) -> None:
        key_press = event.key()
        if key_press == Qt.Key_Space or key_press == Qt.Key_S:
            enterStepMode()
            togglePause()
        elif key_press == Qt.Key_R:
            escapeStepMode()
...
@click.option("--weights", "-w", default='weights', help="Weights Data Folder")
@click.argument("cmd", default='hide', nargs=1)
def main(**kwargs):
    '''main function'''
    global data_folder
    data_folder = kwargs['weights']
    ...
    if kwargs['cmd'] == 'show':
        show = True
        fps = 200
    window = MainWindow(settings, show, fps)
    sys.exit(app.exec_())
